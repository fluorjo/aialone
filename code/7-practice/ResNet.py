...
class Resnet_middle(nn.Module):
    def __int__(self):
        super().__init__()
        #입출력숫자, 블럭 갯수만 넣을 수 있는 함수를 만들어 주면 됨. 
        #64의 입력을 받고 64의 출력을 내보내는 block이 두 개 들어간다
        self.layer1=self.make_layer(64,64,2)
        #64의 입력을 받고 128의 출력을 내보내는 block이 두 개 들어간다
        self.layer2=self.make_layer(64,128,2,True)
        #128의 입력을 받고 256의 출력을 내보내는 block이 두 개 들어간다
        self.layer3=self.make_layer(128,256,2,True)
        #256의 입력을 받고 512의 출력을 내보내는 block이 두 개 들어간다
    ...
    def make_layer(self,in_channel,out_channel,num_block,downsampling=False):
        ...
        #리스트는 별 하나, 딕셔너리는 별 두 개.키와 벨류가 있으니까. 

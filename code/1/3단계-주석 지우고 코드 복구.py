...
#1.토치. 메인 네임스페이스(=변수, 함수 이름과 같은 명칭을 사용하는 공간으로 '소속'을 나타냄)
import torch
#2.
...
train_dataset=MNIST(root="./a",train=True,transform=ToTensor(),download=True)
test_dataset=MNIST(root="./a",train=False,transform=ToTensor(),download=True)
...
model=MLPclass(hidden_size,num_classes).to(device)
#로스 함수. crossentropy
...
for epoch in range(epochs): 
    ...
    for index, (image, target) in enumerate(train_dataloader):
        #이미지와 타겟 디바이스로 전송
        image=image.to(device)
        #image를 모델에 넣어 출력값 만들기.
        out=model(image)
        #출력값과 타겟을 손실함수에 넣어 손실값 계산
        loss_value=lossfunction(out,target)
        #남아있을 수 있는 기울기를 초기화.
        optim.zero_grad()
        #기울기 계산. 역전파.
        loss_value.backward()
